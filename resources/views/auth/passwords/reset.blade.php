...
<main class="wrap">
    ...
    <section class="card hero" aria-labelledby="login-title">
        ...
        <form action="{{ route('password.store') }}" method="POST" id="resetPasswordForm" novalidate>
            @csrf
            @method('PATCH')
            <input type="hidden" name="token" value="{{ $token }}">
            <div class="field">
                <label for="email">メールアドレス</label>
                <input id="email" name="email" type="email" inputmode="email" autocomplete="email" placeholder="you@example.com" required value="{{ $email }}" readonly />
            </div>
            ...
            <div class="field">
                <label for="confirm">新しいパスワード確認</label>
